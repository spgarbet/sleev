...
@tbl-vccchead displays the first six rows of the VCCC dataset:
...
phase one) and `Sex`. The `spline2ph()` function in `sleev` packages can set up the B-spline basis, and combine it with the data input for the final analysis. The column names of the B-spline basis is by default `bs[num]`, where `[num]` is the index of the B-spline basis column.

Here, we use a cubic B-spline basis with the `degree=3` argument in our
call to the `spline2ph()` function from the `splines` package. The size of the
basis $s_{n}$ is set to be 20; this was specified through the `size = 20`
argument. The B-spline basis is set up separately for the two `Sex`
groups by specifying argument `group = "Sex"`. Here, the size of the B-spline basis is assigned in proportion to
...
`linear2ph()` returns an object of class `linear2ph`, denoted by `res_linear`
in the code above. An object of class "linear2ph" is a list containing five slots: `coefficients`,
...
The `coef()` function takes an object of class `linear2ph` and gives the regression coefficient
estimates and their corresponding standard error estimates,
$z$-statistics, and $p$-values. This is analogous to the content of element `coefficients` in the output list.
...
The `res_linear$covariance` slot gives the covariance matrix, and the `summary()` function takes an object of class `linear2ph` and gives both the model parameter estimates and the covariance matrix. The covariance matrix can
...
est.mean <- res_linear_coef[,1]%*%x.vec # calculate estimated mean
est.mean
```
...
res_linear_cov <- summary(res_linear)[[2]]
...
for (i in 1:length(sns)) {
  ...
  res.1 <- cv_linear2ph(y = "CD4_val_sq10", y_unval = "CD4_unval_sq10",
                        ...
                        b_spline = paste0("bs", 1:sn), data = data.sieve, 
...
The average predicted log-likelihoods and run time for the different $s_n$ considered
are:
...
sn.35 <- 35
Bspline <- spline2ph(x = "VL_unval_l10", data = mock.vccc, size = sn.35,
                     degree = 3,  group = "Sex")
...
fit.sn.35 <- linear2ph(y = "CD4_val_sq10", y_unval = "CD4_unval_sq10",
                       x = "VL_val_l10", x_unval = "VL_unval_l10", z = "Sex", 
                       b_spline=paste0("bs", 1:sn.35), data=data.sieve, 
                       hn_scale = 1, se = TRUE, tol = 1e-04, 
                       max_iter = 1000, verbose = FALSE)
paste0("Run time: ", round(difftime(Sys.time(), start.time, 
                                    units = "secs"), 3), " sec")
...
When there are two continuous covariates in the model, the B-spline bases is constructed from 
tensor product of the one-dimensional B-spline bases for each variable.
In this example, the two variables are VL and age, stratified by sex. Due to the curse of
...
is 4. If we set one-dimensional $s_n$ for each sex and each variable to be $4$, the aggregate size of the
...
data.bivariate <- spline2ph(x = c("VL_unval_l10", "Age"), size = 8, degree = 3,
                  data = mock.vccc, group = "Sex",split_group = FALSE)
```
...
res_linear_bivariate = linear2ph(y = "CD4_val_sq10", y_unval = "CD4_unval_sq10",
                       ...
                       z = c("Age", "Sex"), b_spline = paste0("bs", 1:sn),
...
data_pca <- spline2ph(x = "VLAge_pca", size = sn, degree = 3, 
                      data = mock.vccc,group = "Sex", split_group = TRUE)
start.time <- Sys.time()
...
data.logistic <- spline2ph(x = "CD4_unval_sq10", size = 20, degree = 3,
                           data = mock.vccc, group = "prior_ART", split_group = TRUE)
```
...
res_logistic <- logistic2ph(y = "ADE_val", y_unval = "ADE_unval",   
                            ...
                            z = "prior_ART", b_spline = paste0("bs", 1:sn),
