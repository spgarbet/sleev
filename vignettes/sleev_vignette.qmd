...
$Y=\alpha+\pmb{\beta}^{\textup T}\mathbf{X}+\epsilon$, where $\epsilon$
...
model $Y=\alpha+\pmb{X}\pmb{\beta}+\epsilon$. Denote the validation
...
$P_{\pmb{\theta}}(Y=1|\mathbf{X})=\left[1+\exp\{-(\alpha+\pmb{\beta}^{\textup T}\mathbf{X})\}\right]^{-1}$
with parameters
